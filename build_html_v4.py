...
def generate_qa_script(q, a):
    a_clean = a.replace('<br>', ' ').replace('**', '')
    return f"<strong>[{q} 답변 스크립트]</strong><br><br>네, 해당 질문에 대해 설명드리겠습니다.<br><br>{a_clean}<br><br>궁금하신 점이 더 있으시다면 언제든 말씀해 주십시오. 이상입니다."

def generate_overview_script(q, a):
    a_clean = a.replace('<br>', ' ').replace('**', '').replace('■', '')
    a_clean = re.sub(r'<li>(.*?)</li>', r' \1. ', a_clean)
    return f"<strong>[{q} 설명 스크립트]</strong><br><br>이 부분은 전체 시스템 관점에서 설명드리겠습니다.<br><br>{a_clean}<br><br>이상으로 핵심 흐름에 대한 설명을 마칩니다."

def polite(sentence):
    sentence = re.sub(r'(필요|중요|큼|높음|유리)\.$', r'\1합니다.', sentence)
    sentence = re.sub(r'(가짐|있음)\.$', r'있습니다.', sentence)
    sentence = re.sub(r'(한계|제약|부분|어려움)\.$', r'\1입니다.', sentence)
    return sentence

def generate_adr_script(q, raw_content):
    context = re.search(r'### Context\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    options_match = re.search(r'### Options\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    decision = re.search(r'### Decision\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    why_not = re.search(r'### Why Not Others\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    tradeoffs = re.search(r'### Trade-offs?(?: Accepted)?\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    mitigation = re.search(r'### Mitigation\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    revisit = re.search(r'### Revisit Trigger\n(.*?)(?=\n### |$)', raw_content, re.DOTALL)
    ...
    ctx = polite(clean(context))
    opt = clean(options_match).replace('A)', 'A는').replace('B)', 'B는').replace('C)', 'C는').replace('D)', 'D는')
    dec = clean(decision).replace('A) ', '').replace('B) ', '')
    why = polite(clean(why_not))
    trd = polite(clean(tradeoffs))
    mit = polite(clean(mitigation))
    rev = polite(clean(revisit))
    evd = polite(clean(evidence))

    script = f"<strong>[{q} 발표 스크립트]</strong><br><br>"
    if ctx: script += f"이번 결정의 맥락은 {ctx} 라는 점입니다.<br><br>"
    if opt: script += f"그래서 선택지를 다음과 같이 세 가지 정도로 고려했습니다: {opt}.<br><br>"
    if dec: script += f"결론은 <strong>{dec}</strong> (으)로 가겠습니다.<br><br>"
    if why: script += f"그 이유는 {why} 때문입니다.<br><br>"
    if trd: script += f"다만 이 선택에는 트레이드오프가 있습니다. {trd}.<br><br>"
    if mit: script += f"그래서 완화책으로 {mit} 방식을 취하거나 구조를 잡겠습니다.<br><br>"
    if rev: script += f"그리고 이 결정을 다시 볼 트리거도 명확히 하겠습니다. {rev} 시점에 재검토합니다.<br><br>"
    if evd: script += f"마지막으로 이 결정이 말로만 남지 않도록 근거를 문서에 연결해 두었습니다. {evd} 등에서 확인하실 수 있습니다."
    
    script = script.replace('함.', '합니다.').replace('됨.', '됩니다.').replace('임.', '입니다.').replace('음.', '습니다.')
    return script
        ...
        for i in range(1, len(sub_sections), 2):
            ...
            for m in q_matches:
                ...
                items.append({
                    ...
                    "q": q_text,
                    "a": a_text,
                    "script": generate_qa_script(q_text, a_text)
                })
            sections.append({
...
def parse_adr(filepath):
    ...
    for p in parts[1:]:
        ...
        adrs.append({
            ...
            "q": q_text,
            "a": content_html,
            "script": generate_adr_script(q_text, raw_content)
        })
    return adrs
    ...
    for sec_num, sec_title in sections_to_extract:
        ...
        if match:
            ...
            overview_items.append({
                ...
                "q": sec_title,
                "a": content_html,
                "script": generate_overview_script(sec_title, content_html)
            })
...
techTabsData = {
    ...
    "tech5": [item["id"] for item in overviews]
}
...
<head>
    ...
    <style>
        ...
        .script-toggle-btn { margin-top: 1.5rem; background: #f1f5f9; color: #475569; border: 1px dashed #cbd5e1; padding: 0.6rem 1.2rem; border-radius: 6px; font-size: 0.9rem; font-weight: 700; cursor: pointer; transition: all 0.2s; display: inline-flex; align-items: center; gap: 0.5rem; }
        .script-toggle-btn:hover { background: #e2e8f0; color: var(--accent-color); border-color: var(--accent-color); }
        .script-content { margin-top: 1rem; padding: 1.5rem; background: #fffbeb; border-left: 4px solid #f59e0b; border-radius: 6px; font-size: 1.05rem; color: #451a03; line-height: 1.8; animation: fadeIn 0.3s; }
...
<script>
            ...
            <div class="${wrapperClass}">
                ...
                <div class="faq-answer">
                    <div class="answer-content">
                        ...
                        <br>
                        <button class="script-toggle-btn" onclick="toggleScript(this, event)">🎤 발표 스크립트 도우미 열기</button>
    ...
    function toggleAnswer(button) {
        ...
        if (button.classList.contains('active')) {
            // Recalculate full height including the script content if it's open
...
print("Generated HTML with Script Presentation Support!")
